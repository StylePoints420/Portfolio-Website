...
import { SafariMock } from "./safari-mock";
import { useIsMobile } from "@/hooks/use-mobile";
import { Iphone15Mock } from "./iphone15-mock";
...
export const DeckItem = ({ className, ...props }: DeckItemProps) => (
  <div
    className={cn(
      "flex h-full w-full items-center justify-center text-card-foreground ",
      className
    )}
    {...props}
  />
...
export const DeckEmpty = ({
  ...
  return (
    ...
    >
      {(children ?? isMobile) ? (
        <Iphone15Mock src={"Pending..."} className="h-full" />
      ) : (
        <SafariMock
          url={"Pending..."}
